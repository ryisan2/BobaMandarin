  ...
  return (
    <footer
      className={cn(
        ...
        status === "correct" &&
          "border-transparent border-green-500 bg-green-100",
        status === "wrong" && "border-transparent border-rose-500 bg-rose-100"
      )}
    >
      <div
        className=" max-w-[1140px] h-full mx-auto flex 
      ...
      >
        ...
        <Button
          disabled={disabled}
          className="ml-auto border-purple-950 hover:bg-purple-950"
          onClick={onCheck}
          size={isMobile ? "sm" : "lg"}
          variant={status === "wrong" ? "danger" : "secondary"}
        >
          {status === "unanswered" && "Check Answer"}
          {status === "correct" && "Next"}
          {status === "wrong" && "Retry"}
